...
const BOOKS_STORAGE_KEY = 'books_db'
...
var gBooks = utilService.loadFromStorage(BOOKS_STORAGE_KEY) || gDefaultBooks;
...
function addReview(bookId, review) {
  ...
  utilService.storeToStorage(BOOKS_STORAGE_KEY, gBooks)
}
...
return getByIdFromTemp(bookId)
  .then(book => {
    const bookinLocalFormat =  {
      'id': book.id,
    ...
    gBooks.push(bookinLocalFormat);
    utilService.storeToStorage(BOOKS_STORAGE_KEY, gBooks)
    return Promise.resolve(bookinLocalFormat);
  })
}
...
function getByIdFromTemp(id) {
  const book = gTempBooks.find(currBook => currBook.id === id)
  return Promise.resolve(book)
}

function getByIdx(idx) {
  return Promise.resolve(gBooks[idx])
}
...
function getIdxById(id) {
  var bookIdx;
  const book = gBooks.find((currBook, idx) => {
    bookIdx = idx
